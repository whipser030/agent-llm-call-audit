...
import type { HermesLlmAuditRecord, HostLogger } from "./types.js";
...
export async function startHermesLlmAuditMonitor(
  config: Partial<HermesMonitorConfig> = {},
  logger: HostLogger = consoleLogger,
): Promise<HermesMonitorHandle> {
  ...
  const ingestFile = (filePath: string) => ingestJsonlFile(filePath, states, store, logger);
  const ingestAll = () => {
  ...
  let server: LlmAuditServerHandle;
  try {
...
export function resolveHermesMonitorConfig(config: Partial<HermesMonitorConfig> = {}): HermesMonitorConfig {
  return {
    sourceDir: path.resolve(
      expandHome(config.sourceDir ?? process.env.HERMES_LLM_AUDIT_SOURCE_DIR ?? DEFAULT_SOURCE_DIR),
    ),
    auditHome: path.resolve(
      expandHome(config.auditHome ?? process.env.HERMES_LLM_AUDIT_MONITOR_HOME ?? DEFAULT_AUDIT_HOME),
    ),
...
function ingestJsonlFile(
  ...
  if (!/^llm_calls_.*\.jsonl$/.test(path.basename(filePath))) return;
  let stat: fs.Stats;
  try {
...
function ingestLine(
  ...
  if (!line.trim()) return;
  try {
    const parsed = JSON.parse(line) as HermesLlmAuditRecord;
...
function listAuditFiles(sourceDir: string): string[] {
  try {
    return fs
...
function isCompleteJsonObject(line: string): boolean {
  try {
    JSON.parse(line);
    return true;
  } catch {
...
function numberFrom(value: number | undefined, envValue: string | undefined, fallback: number): number {
