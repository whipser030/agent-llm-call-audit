...
function renderLegacyViewer(options: LlmAuditViewerOptions): string {
...
function renderModernShellViewer(options: LlmAuditViewerOptions): string {
  const variant = options.variant === "hermes" ? "hermes" : "openclaw";
  const title = escapeTemplate(
    options.title ??
      (variant === "hermes" ? "Hermes LLM Audit Monitor" : "Openclaw LLM Audit Viewer"),
  );
  const taskPlaceholder = escapeTemplate(
    options.taskPlaceholder ?? (variant === "hermes" ? "Hermes task_id" : "Task ID / runId"),
  );
  const htmlClass = variant === "hermes" ? "audit-shell audit-shell--hermes" : "audit-shell audit-shell--openclaw";
  const hint = "";
  const loadingMsg =
    variant === "hermes" ? "Loading recent Hermes tasks..." : "Loading recent OpenClaw tasks...";
...
function buildModernShellScript(brand: "openclaw" | "hermes", autoLoadLatestTask: boolean): string {
  const L: string[] = [];
