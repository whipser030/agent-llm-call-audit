...
import { startLlmAuditServer, type LlmAuditServerHandle } from "../src/server.js";
import { LlmAuditStore } from "../src/store.js";
import { startHermesLlmAuditMonitor, type HermesMonitorHandle } from "../src/hermes.js";
...
let tmpDir: string;
let store: LlmAuditStore;
let server: LlmAuditServerHandle | null = null;
let hermesMonitor: HermesMonitorHandle | null = null;
...
describe("LlmAuditStore", () => {
  ...
  it("imports Hermes JSONL records once by source key", () => {
    ...
    store.recordHermesAuditRecord(
      { session_id: "session-only", provider: "custom", model: "gpt-5.5" },
      "llm_calls_session-only.jsonl:1",
    );
...
describe("LlmAuditServer", () => {
  it("serves health and Task ID call queries", async () => {
    ...
    const health = await fetchJson<{ ok: boolean; name: string }>(`${server.url}/api/health`);
    expect(health).toMatchObject({ ok: true, name: "Openclaw LLM Audit Viewer" });

    const payload = await fetchJson<{ taskId: string; calls: unknown[] }>(
      `${server.url}/api/tasks/run-http/calls`,
    );
...
describe("Hermes monitor entry", () => {
  it("tails Hermes audit JSONL into the shared viewer store", async () => {
    ...
    fs.writeFileSync(
      path.join(sourceDir, "llm_calls_session-hermes.jsonl"),
      `${JSON.stringify({
        session_id: "session-hermes",
        task_id: "task-hermes",
        provider: "custom",
    ...
    hermesMonitor = await startHermesLlmAuditMonitor(
      { sourceDir, auditHome, port: 0, pollMs: 50 },
      { info: () => {}, warn: () => {}, error: () => {} },
    );

    const payload = await fetchJson<{ taskId: string; calls: unknown[] }>(
      `${hermesMonitor.url}/api/tasks/task-hermes/calls`,
    );
...
describe("Openclaw plugin entry", () => {
  it("registers startup service and LLM hooks", async () => {
    ...
    const handlers = new Map<
      string,
      (event: never, ctx: PluginHookAgentContext) => Promise<void> | void
    >();
    ...
    const api: OpenClawPluginApi = {
      ...
      registerService: (s: RegisteredService) => {
        service = s;
      },
      on: (name, handler) => {
    ...
    plugin.register(api);
    const registeredService = service as RegisteredService | null;
    ...
    const ctx: OpenClawPluginServiceContext = { stateDir: tmpDir };
